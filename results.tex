...
%%<<<<<<< HEAD %from which each fruit originates.  They compute classification
%accuracy %as \textcolor{blue}{finish this.}.  XGBoost also computes MSE for
%its regression task %on the same datasets.  The dataset has been filtered
%authors to remove correlated %features (those with an absolute value of the
%Pearson correlation coefficient greater than 0.8).  %The resulting dataset
%contains 218150 rows (individual fruit) each with 19 features. 

...
%Based on the model trained by default parameter setting, this average MSE metric is 0.284.
...
A hyperparameter configuration for this application is 
...
%svcparams: https://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html
...
The applications train a model on a subset (80\%) of the
...
% Note that this comparison assumes that the memory size used by each benchmark does not depend on hyperparameter settings. We have verified that this assumption holds for the benchmarks and datasets we consider and plan to consider applications for which hyperparameter settings require different maximum amounts of memory as part of future work.  
...
%not sure what this paragraph is adding, seems to a repetition of what
...
%The central rectangle covers from first quartile to third quartile (Q3 - Q1) and the whiskers span from \texttt{$(Q3 + 2 * IQR)$} to \texttt{$(Q1 - 2 * IQR)$}. 
...
% \begin{table}
% \centering
% \input{tables/speedup-tab}
% \caption{The execution time of tuning in seconds by using Seneca and EC2 instance (Serial) and respective speedup. 
% \label{tab:accuracy}}
% \vspace{-0.2in}
% \end{table}

Finally, we compare Seneca to AWS EC2 use.
...
%\textcolor{blue}{FIX THIS: State the results (speed up) per application and 
%also state the cost analysis for 1 hour of computation.  Clarify that this
%is due to the 1-hour pricing model of EC2.}

%We use instance type t2.medium
%for Prophet (since it cannot be built
%process on EC2 instance across five applications. Since library compilation issue, we use t2.medium on Prophet and t2.micro on other 4 applications. 
%As a result, Seneca achieves at least 4x speedup for IO-bound application (SVC) and 21x speedup for CPU-bound application (XGBoost). It confirms Seneca can execute tuning within a more reasonable period of time, comparing to executing serially on EC2 instance.
...
Rows 3--6 show the performance and cost impact of using Seneca's memory optimization.
...
across the five applications (30 runs) is \$0.050. %I use only split 1 for all 5
%numbers are below
...
Overall, given the AWS Lambda cost model and its performance variability, 
...
%Even using the maximum memory available
%for almost 25 minutes of execution 
%time (cf Prophet in Table~\ref{tab:cost_optimized}), the
%total dollar cost is approximately 73 cents.  Secondly, Seneca's memory
%optimizations produce a significant savings to further reduce its cost.
%
%Note that Seneca's memory/cost optimization
%can introduce an execution time penalty (relative to the maximum memory
%execution) for some of the applications.  
%That is, the
...
%To corroborate the cost-effectiveness of Seneca, we analyzed the total cost of five benchmarks in 30 executions from last section. We observe the reported \textit{billed duration} and \textit{max memory used} oscillate that leads to different total cost across 30 executions and we assume it is because the pagination and memory sampling method used by different AWS Lambda host servers. However, even taking fluctuation into account, the 
%highest cost of execution under optimized memory is still cheaper than the lowest cost of execution under maximum memory in all five benchmarks. We believe Seneca's optimizer is economical despite the locality of lambda container and application.
...
%any anomalies (i.e., why is prophet optimization cost more than the others; why
